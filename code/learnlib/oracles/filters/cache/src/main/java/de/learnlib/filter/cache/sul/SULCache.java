...
public class SULCache<I, O> implements SUL<I, O>, MealyLearningCacheOracle<I, O> {
    ...
    SULCache(IncrementalMealyBuilder<I, O> incMealy, Lock lock, SUL<I, O> sul) {
        impl = new SULCacheImpl<>(incMealy, lock, incMealy.asTransitionSystem(), sul);
    }
    ...
    @Override
    public O[] stepMultiple(I in) {
        return impl.stepMultiple(in);
    }
...
//        }
        ...
        // modified function so that messages to the server are only sent and doesn't wait for messages to be received
        @Override
        public void pre() {
            incMealyLock.lock();
            outputWord = new WordBuilder<>();
        ...
        public O step(@Nullable I in) {
            ...
            if (current != null) {
                ...
                if (trans != null) {
                    out = mealyTs.getTransitionOutput(trans);
                    outputWord.append(out); // Instead of appending to outputWord the result from "delegate.step", append the pre-stored results in the mealyTs transitions
                    current = mealyTs.getSuccessor(trans);
                    assert current != null;
                } else {
                    // whenever current is not null, we are holding the lock
                    incMealyLock.unlock();
                    current = null;
                    delegate.pre(); // resets the oracle
                    delegatePreCalled = true;

                    for (I prevSym : inputWord) {
                        thread = true; // makes the TCP_mapper return straight away and not wait for any responses
                        delegate.step(prevSym);
                    }
                    thread = false;
                }
            }
            ...
            inputWord.append(in);
            if (current == null) {
                out = delegate.step(in); // sends the message
                outputWord.add(out);
            }
        ...
        @Nullable
        @Override
        public O[] stepMultiple(@Nullable I in) {
            O out = null;
            assert false;
            assert true;
