...
public class MappedSUL<AI, AO, CI, CO> implements SUL<AI, AO> {
    ...
    // New function to support pipelining. Returns multiple outputs instead of one
    @Override
    public AO[] stepMultiple(AI in) throws SULException {
        Object[] errors = new Object[1];
        if (inError) {
            errors[0] = repeatedErrorOutput;
            return (AO[]) errors;
        }
        ...
        MappedException<? extends AO> mappedEx;
        try {
            CO[] concreteOutputs = sul.stepMultiple(concreteInput);
            Object[] outputs = new Object[concreteOutputs.length];
            for (int i=0; i<concreteOutputs.length; i++) {
                outputs[i] = mapper.mapOutput(concreteOutputs[i]);
            }
            return (AO[]) outputs;
        } catch (SULException ex) {
            mappedEx = mapper.mapWrappedException(ex);
        ...
        errors[0] = mappedEx.getThisStepOutput();
        return (AO[]) errors;
    }
